...
- Add bounded cold-boot recovery using separately supplied, size/hash-pinned
  firmware; protect sensitive transfers and set the SPI buffer to 32,768 bytes.
- Calibrate verification match policy (Policy Version 3) based on offline
  multi-dataset evaluation (342,720 impostor comparisons with 0 observed false
  acceptances under controlled test conditions): tune minimum inliers and mutual
  matches to 5, competing margin to 1, and spatial minimums (span 6x8, cells 2,
  quadrants 1), improving blind-touch genuine matching across dry, tilted, and
  off-center contacts.
...
- Filter unsupported hardware profiles non-destructively during device probe.
- Update automated firmware installer with safe argument passing and
  unprivileged download and verification.
