...
gboolean
fpi_fte3600_brisk_pattern_point (guint   index,
                                 gfloat *x,
                                 gfloat *y)
{
...
gboolean
fpi_fte3600_brisk_descriptor_pair (guint  bit,
                                   guint *first,
                                   guint *second)
{
...
Fte3600BriskStatus
fpi_fte3600_brisk_describe_at (const guint8        *image,
                               gsize                length,
                               gfloat               x,
                               gfloat               y,
                               Fte3600BriskFeature *feature)
...
Fte3600BriskStatus
fpi_fte3600_brisk_extract (const guint8           *image,
                           gsize                   length,
...
fpi_fte3600_brisk_result_meets_diagnostic_policy (const Fte3600BriskMatchResult *result)
{
  ...
  return result->mutual_matches >= FTE3600_BRISK_MIN_MUTUAL_MATCHES &&
         ...
         result->reference_anisotropy >= 0.03;
}
                         ...
                         Fte3600BriskMatchResult      *result)
{
  ...
  result->diagnostic_policy_passed =
    fpi_fte3600_brisk_result_meets_diagnostic_policy (result);
