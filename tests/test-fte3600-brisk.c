...
test_pattern_and_pairs (void)
{
  ...
  g_assert_true (fpi_fte3600_brisk_pattern_point (0, &x, &y));
  g_assert_cmpfloat_with_epsilon (x, 4.0, 1e-6);
  g_assert_cmpfloat_with_epsilon (y, 0.0, 1e-6);
  g_assert_true (fpi_fte3600_brisk_pattern_point (10, &x, &y));
  g_assert_cmpfloat_with_epsilon (x, 8.0, 1e-6);
  g_assert_true (fpi_fte3600_brisk_pattern_point (25, &x, &y));
  g_assert_cmpfloat_with_epsilon (x, 13.0, 1e-6);
  g_assert_false (fpi_fte3600_brisk_pattern_point (45, &x, &y));
  ...
  for (guint bit = 0; bit < FTE3600_BRISK_DESCRIPTOR_BITS; bit++)
    {
      ...
      g_assert_true (fpi_fte3600_brisk_descriptor_pair (bit, &first, &second));
      g_assert_true (fpi_fte3600_brisk_descriptor_pair (bit, &repeat_first, &repeat_second));
    ...
    for (guint i = 0; i < G_N_ELEMENTS (golden_pairs); i++)
      {
        ...
        g_assert_true (fpi_fte3600_brisk_descriptor_pair (golden_pairs[i].bit,
                                                          &first, &second));
...
test_descriptor_deterministic (void)
{
  ...
  g_assert_cmpint (fpi_fte3600_brisk_describe_at (image, sizeof (image), 32.0, 40.0,
                                                  &first),
                   ==, FTE3600_BRISK_OK);
  g_assert_cmpint (fpi_fte3600_brisk_describe_at (image, sizeof (image), 32.0, 40.0,
                                                  &second),
  ...
  g_assert_cmpint (fpi_fte3600_brisk_describe_at (flat, sizeof (flat), 32.0, 40.0,
                                                  &second),
                   ==, FTE3600_BRISK_LOW_CONTRAST);
  g_assert_cmpint (fpi_fte3600_brisk_describe_at (image, sizeof (image) - 1, 32.0, 40.0,
                                                  &second),
                   ==, FTE3600_BRISK_INVALID_ARGUMENT);
  g_assert_cmpint (fpi_fte3600_brisk_describe_at (image, sizeof (image), 4.0, 40.0,
                                                  &second),
                   ==, FTE3600_BRISK_INVALID_ARGUMENT);
}
...
test_extract_deterministic (void)
{
  ...
  make_visual_pattern (image);
  first_status = fpi_fte3600_brisk_extract (image, sizeof (image), &first);
  second_status = fpi_fte3600_brisk_extract (image, sizeof (image), &second);
...
test_rounding_mode_isolation (void)
{
  ...
  make_visual_pattern (image);
  g_assert_true (fpi_fte3600_brisk_pattern_point (17, &point_x_nearest,
                                                  &point_y_nearest));
  g_assert_cmpint (fpi_fte3600_brisk_describe_at (
                     image, sizeof (image), 32.0f, 40.0f,
                     &described_nearest), ==, FTE3600_BRISK_OK);
  g_assert_cmpint (fpi_fte3600_brisk_extract (image, sizeof (image),
                                              &extracted_nearest), ==,
                   FTE3600_BRISK_OK);
  g_assert_true (fpi_fte3600_brisk_validate_feature_set (&extracted_nearest,
                                                         NULL));
  match_status_nearest = fpi_fte3600_brisk_match (&extracted_nearest,
                                                  &extracted_nearest,
                                                  &match_nearest);
  diagnostic_nearest =
    fpi_fte3600_brisk_result_meets_diagnostic_policy (&match_nearest);
  ...
  for (guint i = 0; i < G_N_ELEMENTS (modes); i++)
    {
      ...
      g_assert_true (fpi_fte3600_brisk_pattern_point (17, &point_x, &point_y));
      ...
      g_assert_true (fpi_fte3600_brisk_validate_feature_set (&extracted, NULL));
      g_assert_cmpint (fegetround (), ==, modes[i]);

      match_status = fpi_fte3600_brisk_match (&extracted, &extracted, &match);
      g_assert_cmpint (fegetround (), ==, modes[i]);
      g_assert_cmpint (match_status, ==, match_status_nearest);
      g_assert_cmpmem (&match, sizeof (match),
                       &match_nearest, sizeof (match_nearest));
      g_assert_cmpint (fpi_fte3600_brisk_result_meets_diagnostic_policy (&match),
                       ==, diagnostic_nearest);
      g_assert_cmpint (fegetround (), ==, modes[i]);
      ...
      g_assert_true (fpi_fte3600_brisk_validate_feature_set (&boundary, NULL));
      g_assert_cmpint (fegetround (), ==, modes[i]);
      boundary.features[0].orientation =
        -FTE3600_BRISK_ORIENTATION_LIMIT;
      g_assert_true (fpi_fte3600_brisk_validate_feature_set (&boundary, NULL));
      ...
      g_assert_false (fpi_fte3600_brisk_validate_feature_set (&boundary, NULL));
      boundary.features[0].orientation =
        nextafterf (-FTE3600_BRISK_ORIENTATION_LIMIT, -INFINITY);
      g_assert_false (fpi_fte3600_brisk_validate_feature_set (&boundary, NULL));
...
test_rigid_consensus (void)
{
  ...
  g_assert_cmpint (fpi_fte3600_brisk_match (&query, &reference, &result),
                   ==, FTE3600_BRISK_OK);
  ...
  g_assert_true (fpi_fte3600_brisk_result_meets_diagnostic_policy (&result));
}
...
test_end_to_end_translation (void)
{
  ...
  g_assert_cmpint (fpi_fte3600_brisk_extract (source, sizeof (source), &query),
                   ==, FTE3600_BRISK_OK);
  g_assert_cmpint (fpi_fte3600_brisk_extract (translated, sizeof (translated), &reference),
                   ==, FTE3600_BRISK_OK);
  {
...
test_end_to_end_rotation (void)
{
  ...
  g_assert_cmpint (fpi_fte3600_brisk_extract (source, sizeof (source), &query),
                   ==, FTE3600_BRISK_OK);
  g_assert_cmpint (fpi_fte3600_brisk_extract (transformed, sizeof (transformed),
                                              &reference),
                   ==, FTE3600_BRISK_OK);
  g_assert_cmpint (fpi_fte3600_brisk_match (&query, &reference, &result),
                   ==, FTE3600_BRISK_OK);
...
test_incoherent_geometry_rejected (void)
{
  ...
  status = fpi_fte3600_brisk_match (&query, &reference, &result);
...
test_malformed_sets_rejected (void)
{
  ...
  g_assert_false (fpi_fte3600_brisk_result_meets_diagnostic_policy (&result));
  assert_authentication_policy_result (&result);
}
