...
typedef struct
{
  ...
  const gchar *irq_controller_path;
...
static const TestPlatform platforms[] = {
  { "ONE-NETBOOK TECHNOLOGY CO., LTD.", "A1",
    "\\_SB_.PCI0.GPI0", "\\_SB_.PCI0.GPI0", "", "", 0x55, 0x56 },
  { "MEDION", "E3224", "\\_SB_.GPO1", "\\_SB_.GPO2", "FT", "YS13G", 0x27, 0x00 },
};

static gint mock_chips[2];
static gint mock_requests[2];
                            ...
                            gsize *length, GError **error)
{
  ...
  else if (g_str_equal (path, "/sys/class/dmi/id/product_version"))
    value = sensor.platform->product_version;
  else if (g_str_equal (path, "/sys/class/dmi/id/board_name"))
    value = sensor.platform->board_name;
  else if (g_str_equal (path, "/mock/gpio-reset/firmware_node/path") ||
           g_str_equal (path, "/mock/gpio-reset/device/firmware_node/path"))
    value = sensor.platform->controller_path;
  else if (g_str_equal (path, "/mock/gpio-irq/firmware_node/path") ||
           g_str_equal (path, "/mock/gpio-irq/device/firmware_node/path"))
    value = sensor.platform->irq_controller_path;
                                         ...
                                         const gchar *subsystem)
{
  ...
  GObject *reset = g_object_new (G_TYPE_OBJECT, NULL);
  GObject *irq = g_object_new (G_TYPE_OBJECT, NULL);
  GList *devices = NULL;

  g_object_set_data (irq, "irq-controller", GUINT_TO_POINTER (1));
  devices = g_list_append (devices, reset);
  return g_list_append (devices, irq);
}
...
{
  guint index = g_str_equal (path, "/mock/gpiochip-irq") ? 1 : 0;

  g_assert_true (g_str_equal (path, "/mock/gpiochip-reset") || index == 1);
  sensor.chip_opens++;
  return (struct gpiod_chip *) &mock_chips[index];
}

void
__wrap_gpiod_chip_close (struct gpiod_chip *chip)
{
  g_assert_true (chip == (struct gpiod_chip *) &mock_chips[0] ||
                 chip == (struct gpiod_chip *) &mock_chips[1]);
}
                                 ...
                                 struct gpiod_line_config    *line_config)
{
  ...
  g_assert_cmpuint (gpiod_line_config_get_num_configured_offsets (line_config), ==, 1);
  g_assert_cmpuint (gpiod_line_config_get_configured_offsets (line_config, &offset, 1), ==, 1);
  settings = gpiod_line_config_get_line_settings (line_config, offset);
  g_assert_nonnull (settings);
  reset = gpiod_line_settings_get_direction (settings) == GPIOD_LINE_DIRECTION_OUTPUT;
  index = reset ? 0 : 1;
  ...
  if (sensor.fail_claim || (!reset && sensor.fail_irq_claim))
    {
      errno = EBUSY;
      return NULL;
    }
  g_assert_cmpuint (sensor.claimed & (1u << index), ==, 0);
  sensor.claimed |= 1u << index;
  sensor.claims++;
  return (struct gpiod_line_request *) &mock_requests[index];
}

void
__wrap_gpiod_line_request_release (struct gpiod_line_request *request)
{
  guint index = request == (struct gpiod_line_request *) &mock_requests[0] ? 0 : 1;

  g_assert_true (request == (struct gpiod_line_request *) &mock_requests[index]);
  g_assert_cmpuint (sensor.claimed & (1u << index), !=, 0);
  if (index == 0)
...
{
  g_assert_true (request == (struct gpiod_line_request *) &mock_requests[0]);
  g_assert_cmpuint (sensor.claimed & 1, !=, 0);
...
{
  g_assert_true (request == (struct gpiod_line_request *) &mock_requests[1]);
...
test_open_error (gconstpointer data)
{
  ...
  sensor.fail_irq_claim = failure == 3;
  ...
  sensor.fail_config = sensor.fail_claim = sensor.bad_id = sensor.fail_irq_claim = FALSE;
  open_device (device);
  finish_device (device);
}
...
main (int argc, char **argv)
{
  ...
  g_test_add_data_func ("/fte3600-lifecycle/open/id-error", GUINT_TO_POINTER (2),
                        test_open_error);
  g_test_add_data_func ("/fte3600-lifecycle/open/second-gpio-error", GUINT_TO_POINTER (3),
                        test_open_error);
  return g_test_run ();
}
